...
namespace javascripttest.DbHelper
{
    public class SqliteHelper
    {
        ...
        public static int ExecuteNonQuery(string connectionString, List<string> commandTextList, CommandType commandType)
        {
            int result = 0;
            if (connectionString == null || connectionString.Length == 0)
                throw new ArgumentNullException("connectionString");
            ...
            using (SQLiteConnection con = new SQLiteConnection(connectionString))
            {
                ...
                try
                {
                    foreach (var commandText in commandTextList)
                    {
                        PrepareCommand(cmd, con, ref trans, true, commandType, commandText);
                        result = cmd.ExecuteNonQuery();
                        trans.Commit();
